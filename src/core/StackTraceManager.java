...
public class StackTraceManager {
	...
	protected ArrayList<QueryPhrase> calculateFinalScores()
	{
		...
		for(String key:sortedMap.keySet()){
			if(this.stopList.contains(key.toLowerCase()))continue;
			tokens.add(key);
			count++;
			if(count==5)break;
		}
	...
	protected void populateStopList()
	{
		...
		try{
			String content=ContentLoader.downloadPageContent("http://homepage.usask.ca/~masud.rahman/contentsuggest/stop.txt");
