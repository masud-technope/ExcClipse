...
import query.code.MyQueryMaker;
import style.JavaLineStyler;
import core.code.CodeDisplayManager;
import core.code.Result;
import core.code.SearchEventManager;
import ca.usask.cs.srlab.excclipse.handlers.code.ViewContentProvider;


public class SurfExampleClientView extends ViewPart {

	public TableViewer viewer;
	public static final String ID = "ca.usask.cs.srlab.excclipse.views.SurfExampleClientView";
	...
	protected void addSearchPanel(Composite parent) {
		...
		refreshButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				MyQueryMaker qmaker=new MyQueryMaker(manager.exceptionName, manager.contextCode, manager.selectedLine);
	...
	protected void addResultTable(Composite parent) {
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				//System.out.println(result.completeCode);
				//sourceViewer.setDocument(new Document(result.completeCode));
			}
		});
		
		/*viewer.getTable().addListener(SWT.SINGLE, new Listener() {
			@Override
			public void handleEvent(Event event) {
				// TODO Auto-generated method stub
				codeViewer.setText("");
				TableItem item=viewer.getTable().getItem(event.index);
		...
		codeViewer=new StyledText(divider, SWT.BORDER | SWT.MULTI | SWT.V_SCROLL
		        | SWT.H_SCROLL);
	...
	protected void setItemHeight(Table table)
	{
		table.addListener(SWT.MeasureItem, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				event.height = 25;// Math.max(min, size.y + TEXT_MARGIN);
			}
		});
